...
describe("AccountManager", () => {
  ...
  describe("header style fallback for Gemini", () => {
    ...
    it("getAvailableHeaderStyle returns antigravity-cli when antigravity is rate-limited", () => {
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
      ...
      expect(manager.getAvailableHeaderStyle(account!, "gemini")).toBe("antigravity-cli");
    });

    it("getAvailableHeaderStyle returns null when all header styles are rate-limited", () => {
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
    ...
    it("getMinWaitTimeForFamily considers all Gemini header styles", () => {
      vi.useFakeTimers();
      vi.setSystemTime(new Date(0));
  ...
  describe("soft quota threshold", () => {
    it("skips account over soft quota threshold in sticky mode", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: { remainingFraction: 0.05 } });
    ...
    it("allows account under soft quota threshold", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: { remainingFraction: 0.15 } });
    ...
    it("threshold of 100 disables soft quota protection", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: { remainingFraction: 0.01 } });
    ...
    it("returns null when all accounts over threshold", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: { remainingFraction: 0.05 } });
      manager.updateQuotaCache(1, { claude: { remainingFraction: 0.08 } });

      const account = manager.getCurrentOrNextForFamily("claude", null, "sticky", "antigravity", false, 90);
      expect(account).toBeNull();
    });

    it("round-robin rotates without checking soft quota", () => {
      const stored: AccountStorageV4 = {
        version: 4,
        accounts: [
          { refreshToken: "r1", projectId: "p1", addedAt: 1, lastUsed: 0 },
    ...
    it("handles remainingFraction of 0 (fully exhausted)", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: { remainingFraction: 0 } });
    ...
    it("fails open when cachedQuotaUpdatedAt is missing", () => {
      ...
      acc.cachedQuota = { claude: { remainingFraction: 0.05 } };
  ...
  describe("getMinWaitTimeForSoftQuota", () => {
    ...
    it("returns minimum wait time across multiple accounts", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { 
        claude: { remainingFraction: 0.05,           resetTime: "2026-01-28T15:00:00Z",
        } 
      });
      manager.updateQuotaCache(1, { 
         claude: { remainingFraction: 0.08, resetTime: "2026-01-28T12:00:00Z" }
      });
