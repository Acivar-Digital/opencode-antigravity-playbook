...
If you're seeing unusual rate limit behavior, please share in a [GitHub issue](https://github.com/Acivar-Digital/opencode-antigravity-playbook/issues):
...
3. **Manual callback extraction** (advanced):
   ...
   - See [issue #119](https://github.com/Acivar-Digital/opencode-antigravity-playbook/issues/119) for manual auth support
...
```bash
npx tsx scripts/test-regression.ts --sanity      # 7 tests, ~5 min
npx tsx scripts/test-regression.ts --heavy       # 4 tests, ~30 min
npx tsx scripts/test-regression.ts --dry-run     # List tests
```
...
Open an issue on [GitHub](https://github.com/Acivar-Digital/opencode-antigravity-playbook/issues).
